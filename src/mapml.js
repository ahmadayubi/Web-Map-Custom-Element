...
M.Util = {
  convertAndFormatPCRS : function(pcrsBounds, map){
    ...
    for(let i = 0;i<map.options.crs.options.resolutions.length;i++){
      ...
      let minConverted = map.options.crs.transformation.transform(pcrsBounds.min,scale);
      let maxConverted = map.options.crs.transformation.transform(pcrsBounds.max,scale);
      tcrsTopLeft.push({
